...
@register_tool()
def gmail_search(query: str, max_results: int = 10, account: str | None = None) -> list[dict]:
...
@register_tool()
def gmail_get_message(thread_id: str, account: str | None = None) -> dict:
...
@register_tool()
def gmail_labels(account: str | None = None) -> list[dict]:
...
@register_tool()
def drive_list(
    folder_id: str | None = None, max_results: int = 20, account: str | None = None
) -> list[dict]:
    ...
    response = service.list_files(parent_id=folder_id, max_results=max_results)
...
@register_tool()
def drive_search(query: str, max_results: int = 20, account: str | None = None) -> list[dict]:
    """Search for files in Google Drive.

    Args:
        query: Plain search term (e.g., 'report', 'resume.pdf')
        max_results: Maximum number of files to return
        account: Google account email
    ...
    response = service.search_files(query=query, max_results=max_results)
    files = response.get("files", [])
...
@register_tool()
def drive_get_file(file_id: str, account: str | None = None) -> dict:
...
@register_tool(destructive=True)
def drive_share(file_id: str, email: str, role: str = "reader", account: str | None = None) -> dict:
...
@register_tool()
def calendar_events(
    days: int = 7, calendar_id: str = "primary", account: str | None = None
) -> list[dict]:
...
@register_tool()
def calendar_search(query: str, days: int = 30, account: str | None = None) -> list[dict]:
...
@register_tool()
def tasks_lists(account: str | None = None) -> list[dict]:
    """List all task lists.
...
@register_tool()
def tasks_list(
    tasklist_id: str, show_completed: bool = False, account: str | None = None
) -> list[dict]:
...
@register_tool(destructive=True)
def tasks_complete(tasklist_id: str, task_id: str, account: str | None = None) -> dict:
